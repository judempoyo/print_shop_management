    ...
    <div class="p-6 bg-white rounded-xl shadow-lg dark:bg-gray-800 transition-all duration-300 hover:shadow-xl">
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">
                ...
                Modifier la Séance Photo
            </h1>
        </div>
        
        <form action="<?= PUBLIC_URL ?>photo-session/update/<?= $session->id ?>" method="POST" class="space-y-6">
            <!-- Date et Heure -->
            <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date et Heure</label>
                <div class="relative">
                    <input type="datetime-local" name="date" value="<?= date('Y-m-d\TH:i', strtotime($session->date)) ?>" required
                ...
                <div class="relative">
                    <select name="type" required
                        ...
                        <?php foreach ($sessionTypes as $sessionType): ?>
                            <option value="<?= $sessionType ?>" <?= $session->type === $sessionType ? 'selected' : '' ?>>
                ...
                <div class="relative">
                    <select name="customer_id" required
                        ...
                        <?php foreach ($customers as $customer): ?>
                            <option value="<?= $customer->id ?>" <?= $session->customer_id == $customer->id ? 'selected' : '' ?>>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" rows="3"
                          class="block w-full px-4 py-3 text-gray-900 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white transition duration-300"><?= htmlspecialchars($session->notes) ?></textarea>
            </div>
