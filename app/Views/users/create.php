<div class="max-w-2xl mx-auto mt-8">
    <div class="p-6 bg-white rounded-xl shadow-lg dark:bg-gray-800 transition-all duration-300 hover:shadow-xl">
        ...
        <form action="<?= PUBLIC_URL ?>user/store" method="POST" class="space-y-6">
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input type="text" name="name" required
                           ...
                           value="<?= htmlspecialchars($this->session->get('old')['name'] ?? '') ?>">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input type="tel" name="phone"
                           ...
                           value="<?= htmlspecialchars($this->session->get('old')['phone'] ?? '') ?>">
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="space-y-2">
                ...
                <select name="role" required
                    ...
                    <option value="user" <?= ($this->session->get('old')['role'] ?? '') === 'user' ? 'selected' : '' ?>>Utilisateur standard</option>
                    <option value="admin" <?= ($this->session->get('old')['role'] ?? '') === 'admin' ? 'selected' : '' ?>>Administrateur</option>
                </select>
