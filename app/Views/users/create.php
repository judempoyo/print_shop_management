        ...
        <form action="<?= PUBLIC_URL ?>user/store" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input type="email" name="email" required
                           ...
                           value="<?= htmlspecialchars($sessionManager->get('old')['email'] ?? '') ?>">
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="space-y-2">
                ...
                <select name="role" required
                    ...
                    <option value="user" <?= ($sessionManager->get('old')['role'] ?? '') === 'user' ? 'selected' : '' ?>>Utilisateur standard</option>
                    <option value="admin" <?= ($sessionManager->get('old')['role'] ?? '') === 'admin' ? 'selected' : '' ?>>Administrateur</option>
                </select>
